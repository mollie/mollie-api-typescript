# EntityOrganizationLocale

The preferred locale of the merchant, as set in their Mollie dashboard.

## Example Usage

```typescript
import { EntityOrganizationLocale } from "mollie-api-typescript/models";

let value: EntityOrganizationLocale = "en_US";

// Open enum: unrecognized values are captured as Unrecognized<string>
```

## Values

```typescript
"ca_ES" | "cs_CZ" | "da_DK" | "de_AT" | "de_CH" | "de_DE" | "de_LU" | "en_BE" | "en_GB" | "en_NL" | "en_US" | "es_ES" | "fi_FI" | "fr_BE" | "fr_FR" | "fr_LU" | "hu_HU" | "is_IS" | "it_IT" | "lt_LT" | "lv_LV" | "nb_NO" | "nl_BE" | "nl_NL" | "pl_PL" | "pt_PT" | "sk_SK" | "sv_SE" | Unrecognized<string>
```
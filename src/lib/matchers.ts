...
import { ClientDefaultError } from "../models/errors/clientdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ClientDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ClientDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ClientDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ClientDefaultError("API error occurred", {
          request,
          response,
          body,
        }),

 ...
 */
export function draftTransfersCreate(
  client: ClientCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateDraftTransferRequest$outboundSchema.optional().parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload?.["create-draft-transfer-request"], {
    explode: true,
  });

  const path = pathToFunc("/v2/business-accounts/draft-transfers")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, models.DraftTransferResponse$inboundSchema, {

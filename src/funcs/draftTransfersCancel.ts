 ...
 */
export function draftTransfersCancel(
  client: ClientCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CancelDraftTransferRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    draftTransferId: encodeSimple("draftTransferId", payload.draftTransferId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc(
    "/v2/business-accounts/draft-transfers/{draftTransferId}",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.DraftTransferResponse$inboundSchema, {

 ...
 */
export function accountsGetAccount(
  client: ClientCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetBusinessAccountRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    businessAccountId: encodeSimple(
      "businessAccountId",
      payload.businessAccountId,
      { explode: false, charEncoding: "percent" },
    ),
  };
  const path = pathToFunc("/v2/business-accounts/accounts/{businessAccountId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.BusinessAccountResponse$inboundSchema, {
